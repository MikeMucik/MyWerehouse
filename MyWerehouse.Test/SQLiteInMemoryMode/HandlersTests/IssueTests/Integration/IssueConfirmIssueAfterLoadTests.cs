...
{
	public class IssueConfirmIssueAfterLoadTests : TestBase
	{
		...
		[Fact]
		public async Task ConfirmIssueAfterLoading_ShouldUpdateInventory_WhenValidOk()
		{
			//Arrange
			...
			var product1 = CreateProduct("Prod2", "SKU2", 1);
			...
			//Act
			var result = await Mediator.Send(new ConfirmIssueAfterLoadingCommand(issue.Id, "UserTest"));
			//Assert
			...
			var palletHistory = await DbContext.HistoryPallet
				.Include(p => p.HistoryPalletDetails)
				.FirstAsync(ph => ph.PalletId == pallet.Id);
			Assert.NotNull(palletHistory);
			var palletHistory1 = await DbContext.HistoryPallet
				.Include(p => p.HistoryPalletDetails)
				.FirstAsync(ph => ph.PalletId == pallet1.Id);
			Assert.NotNull(palletHistory1);
		}
		[Fact]
		public async Task ConfirmIssueAfterLoading_ReturnExceptionInventory_WhenInvalidInventory()
		{
			//Arrange
			var client = CreateClient();
			var category = CreateCategory("Cat");
		...
		[Fact]
		public async Task ConfirmIssueAfterLoading_ReturnInfoIssueNotFound_WhenIssueNotExist()
		{
			var receiptId9 = Guid.Parse("91111111-1111-1111-1111-111111111111");
			//Act
			var result = await Mediator.Send(new ConfirmIssueAfterLoadingCommand(receiptId9, "userX"));
			//Assert
			Assert.NotNull(result);
		...
		public async Task ConfirmIssueAfterLoading_ThrowExceptionNotEndedLoading_WhenNotAllPalletIsLoaded()
		{
			...
			await DbContext.SaveChangesAsync();
			//Act&Assert
			var ex = await Assert.ThrowsAsync<NotEndedLoadingDomainException>(() => Mediator.Send(new ConfirmIssueAfterLoadingCommand(issue.Id, "UserTest")));

			Assert.Equal($"Issue {issue.IssueNumber}({issueId}) has pallets not fully loaded.", ex.Message);
		}
		[Fact]
		public async Task ConfirmIssueAfterLoading_ThrowExceptionWrongStatus_WhenShippedIsNotEnded()
		{
			//Arrange
			var client = CreateClient();
			var category = CreateCategory("Cat");
			...
			await DbContext.SaveChangesAsync();
			//Act&Assert
			var ex = await Assert.ThrowsAsync<NotAllowedOperationDomainException>(async () => await Mediator.Send(new ConfirmIssueAfterLoadingCommand(issue.Id, "userX")));

			Assert.Equal($"Operation forbidden for {issue.IssueNumber}({issueId}), wrong status.", ex.Message);
		}
		[Fact]
		public async Task ConfirmIssueAfterLoading_ShouldUpdateStatusAndHistory_WhenTwoProducts()
		{
			// Arrange
			var client = CreateClient();
			var category = CreateCategory("Cat");
			...
			// Act
			var result = await Mediator.Send(new ConfirmIssueAfterLoadingCommand(issue.Id, "user1"));
			// Assert
			Assert.NotNull(result);
			Assert.True(result.IsSuccess);
			Assert.Equal("Załadunek zatwierdzony, zasoby uaktulanione.", result.Message);
