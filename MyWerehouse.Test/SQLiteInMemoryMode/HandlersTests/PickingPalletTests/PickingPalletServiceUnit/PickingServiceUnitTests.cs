...
namespace MyWerehouse.Test.SQLiteInMemoryMode.HandlersTests.PickingPalletTests.PickingPalletServiceUnit
{
	public class PickingServiceUnitTests : TestBase
	{
		...
		public async Task GetListToPicking_ShouldGroupByClientIssueAndProduct()
		{
			...
			pallet1.AddProductForTests(product1.Id, 40, new DateTime(2025, 8, 8), DateOnly.FromDateTime(TestDates.UtcNow.AddDays(365)));

			var pallet2 = Pallet.CreateForTests("Q11", new DateTime(2025, 9, 9), 2, PalletStatus.ToPicking, null, null);
			pallet2.AddProductForTests(product1.Id, 50, new DateTime(2025, 9, 9), DateOnly.FromDateTime(TestDates.UtcNow.AddDays(365)));

			var pallet3 = Pallet.CreateForTests("Q12", new DateTime(2025, 10, 10), 3, PalletStatus.ToPicking, null, null);
			pallet3.AddProductForTests(product2.Id, 70, new DateTime(2025, 10, 10), DateOnly.FromDateTime(TestDates.UtcNow.AddDays(365)));

			var pallet4 = Pallet.CreateForTests("Q13", new DateTime(2025, 11, 11), 4, PalletStatus.ToPicking, null, null);
			pallet4.AddProductForTests(product2.Id, 100, new DateTime(2025, 11, 11), DateOnly.FromDateTime(TestDates.UtcNow.AddDays(365)));

			var issue1Id = Guid.NewGuid();
			var issue1 = Issue.CreateForSeed(issue1Id, 1, 1, TestDates.UtcNow,
			DateOnly.FromDateTime(TestDates.UtcNow.AddDays(7)), "TestUser", IssueStatus.New, null);

			var issue2Id = Guid.NewGuid();
			var issue2 = Issue.CreateForSeed(issue2Id, 2, 1, TestDates.UtcNow,
			DateOnly.FromDateTime(TestDates.UtcNow.AddDays(7)), "TestUser", IssueStatus.New, null);

			var issue3Id = Guid.NewGuid();
			var issue3 = Issue.CreateForSeed(issue3Id, 3, 2, TestDates.UtcNow,
			DateOnly.FromDateTime(TestDates.UtcNow.AddDays(7)), "TestUser", IssueStatus.New, null);
			...
			var a11 = PickingTask.CreateForSeed(pickingGuid11, virtualPallet1.Id, issue1Id, 10, PickingStatus.Allocated, product1.Id, null, null, DateOnly.FromDateTime(TestDates.UtcNow.AddDays(5)), 0);
			var pickingGuid12 = Guid.NewGuid();
			var a12 = PickingTask.CreateForSeed(pickingGuid12, virtualPallet1.Id, issue2Id, 15, PickingStatus.Allocated, product1.Id, null, null, DateOnly.FromDateTime(TestDates.UtcNow.AddDays(5)), 0);
			...
			var a21 = PickingTask.CreateForSeed(pickingGuid21, virtualPallet2.Id, issue1Id, 20, PickingStatus.Allocated, product1.Id, null, null, DateOnly.FromDateTime(TestDates.UtcNow.AddDays(5)), 0);
			var pickingGuid22 = Guid.NewGuid();
			var a22 = PickingTask.CreateForSeed(pickingGuid22, virtualPallet2.Id, issue3Id, 25, PickingStatus.Allocated, product1.Id, null, null, DateOnly.FromDateTime(TestDates.UtcNow.AddDays(5)), 0);
			...
			var a31 = PickingTask.CreateForSeed(pickingGuid31, virtualPallet3.Id, issue2Id, 15, PickingStatus.Allocated, product2.Id, null, null, DateOnly.FromDateTime(TestDates.UtcNow.AddDays(5)), 0);
			...
			var a41 = PickingTask.CreateForSeed(pickingGuid41, virtualPallet4.Id, issue1Id, 10, PickingStatus.Allocated, product2.Id, null, null, DateOnly.FromDateTime(TestDates.UtcNow.AddDays(5)), 0);
			...
			var result = await Mediator.Send(
				...
				(DateOnly.FromDateTime(TestDates.UtcNow.AddDays(4)), DateOnly.FromDateTime(TestDates.UtcNow.AddDays(5))));
		...
		public async Task GetListIssueToPickingAsync_ShouldGroupByClientIssueAndProduct()
		{
			...
			DbContext.SaveChanges();
			var issue1Id = Guid.NewGuid();
			var issue1 = Issue.CreateForSeed(issue1Id, 101, 1, TestDates.UtcNow,
			DateOnly.FromDateTime(TestDates.UtcNow.AddDays(7)), "TestUser", IssueStatus.New, null);
			var issue2Id = Guid.NewGuid();
			var issue2 = Issue.CreateForSeed(issue2Id, 102, 1, TestDates.UtcNow,
			DateOnly.FromDateTime(TestDates.UtcNow.AddDays(7)), "TestUser", IssueStatus.New, null);
			var issue3Id = Guid.NewGuid();
			var issue3 = Issue.CreateForSeed(issue3Id, 103, 2, TestDates.UtcNow,
			DateOnly.FromDateTime(TestDates.UtcNow.AddDays(7)), "TestUser", IssueStatus.New, null);
			var pallet1 = Pallet.CreateForTests("Q10", new DateTime(2025, 8, 8), 1, PalletStatus.Available, null, null);
			pallet1.AddProductForTests(product1.Id, 40, new DateTime(2025, 8, 8), DateOnly.FromDateTime(TestDates.UtcNow.AddDays(365)));
			var pallet2 = Pallet.CreateForTests("Q11", new DateTime(2025, 9, 9), 2, PalletStatus.ToPicking, null, null);
			pallet2.AddProductForTests(product1.Id, 50, new DateTime(2025, 9, 9), DateOnly.FromDateTime(TestDates.UtcNow.AddDays(365)));
			var pallet3 = Pallet.CreateForTests("Q12", new DateTime(2025, 10, 10), 3, PalletStatus.ToPicking, null, null);
			pallet3.AddProductForTests(product1.Id, 70, new DateTime(2025, 10, 10), DateOnly.FromDateTime(TestDates.UtcNow.AddDays(365)));
			var pallet4 = Pallet.CreateForTests("Q13", new DateTime(2025, 11, 11), 4, PalletStatus.ToPicking, null, null);
			pallet4.AddProductForTests(product2.Id, 100, new DateTime(2025, 11, 11), DateOnly.FromDateTime(TestDates.UtcNow.AddDays(365)));
			DbContext.Pallets.AddRange(pallet1, pallet2, pallet3, pallet4);
			DbContext.Issues.AddRange(issue1, issue2, issue3);
			...
			var virtualPallet3 = VirtualPallet.CreateForSeed(Guid.NewGuid(), pallet3.Id, 50, location3.Id, new DateTime(2025, 8, 12));
			var pickingGuid31 = Guid.NewGuid();
			var a31 = PickingTask.CreateForSeed(pickingGuid31, virtualPallet3.Id, issue3Id, 15, PickingStatus.Allocated, product2.Id, null, null, DateOnly.FromDateTime(TestDates.UtcNow.AddDays(5)), 0);
			...
			var result = await Mediator.Send(new GetListIssueToPickingQuery(
				DateOnly.FromDateTime(TestDates.UtcNow.AddDays(3)),
