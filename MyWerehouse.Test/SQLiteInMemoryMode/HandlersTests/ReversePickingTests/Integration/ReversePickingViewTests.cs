	...
	public class ReversePickingViewTests : TestBase
	{
		...
		public async Task GetListReversePickingToDo_ShouldReturnList_WhenOneTaskDone()
		{
			...
			var createIssueDto = new CreateIssueDTO
			{
				...
				{
					new IssueItemDTO { ProductId = product.Id, Quantity = 18, BestBefore = DateOnly.FromDateTime(TestDates.UtcNow.AddDays(366)) }
				}
			};
			...
			//Act 4 
			var resultGetView = await Mediator.Send(new GetListReversePickingToDoQuery(1, 1, DateOnly.FromDateTime(TestDates.UtcNow.AddDays(-1)), DateOnly.FromDateTime(TestDates.UtcNow.AddDays(1))));
			//Assert 4
		...
		public async Task GetListReversePickingToDo_ShouldReturnList_WhenTwoPickingTaskDone()
		{
			...
			var pallet2 = Pallet.CreateForTests("P2", TestDates.UtcNow, 2, PalletStatus.Available, receipt.Id, null);
			pallet2.AddProduct(product.Id, 1, TestDates.UtcNow, DateOnly.FromDateTime(TestDates.UtcNow.AddDays(366)));

			var pallet3 = Pallet.CreateForTests("P3", TestDates.UtcNow, 3, PalletStatus.Available, receipt.Id, null);
			pallet3.AddProduct(product.Id, 7, TestDates.UtcNow, DateOnly.FromDateTime(TestDates.UtcNow.AddDays(366)));
			...
			//Act 4 
			var resultGetView = await Mediator.Send(new GetListReversePickingToDoQuery(10, 1, DateOnly.FromDateTime(TestDates.UtcNow.AddDays(-1)), DateOnly.FromDateTime(TestDates.UtcNow.AddDays(1))));
			//Assert 4
			Assert.NotNull(resultGetView);
		...
		public async Task GetReturnReverseTask_ShouldReturnInfoToExistOptionsAvailable()
		{
			...
			var pallet2 = Pallet.CreateForTests("P2", TestDates.UtcNow, 2, PalletStatus.Available, receipt.Id, null);
			pallet2.AddProduct(product.Id, 8, TestDates.UtcNow, DateOnly.FromDateTime(TestDates.UtcNow.AddDays(366)));

			var pallet3 = Pallet.CreateForTests("P3", TestDates.UtcNow, 3, PalletStatus.Available, receipt.Id, null);
			pallet3.AddProduct(product.Id, 7, TestDates.UtcNow, DateOnly.FromDateTime(TestDates.UtcNow.AddDays(366)));

			var pallet4 = Pallet.CreateForTests("P4", TestDates.UtcNow, 4, PalletStatus.Available, receipt.Id, null);
			pallet4.AddProduct(product.Id, 5, TestDates.UtcNow, DateOnly.FromDateTime(TestDates.UtcNow.AddDays(366)));
		...
		public async Task ListPalletsForForkLifterReservePicking_ReturnList()
		{
			...
			Assert.Equal(pickingPallet.Id, pickingPalletToTakeDown.PalletId);
			Assert.Equal(pickingPallet.PalletNumber, pickingPalletToTakeDown.PalletNumber);
			Assert.Equal(100100, pickingPalletToTakeDown.LocationId);
			Assert.Equal("1-1-6-1", pickingPalletToTakeDown.LocationName);
			Assert.Equal(PalletStatus.Picking, pickingPalletToTakeDown.Status);
		}
	}
}
