...
namespace MyWerehouse.Test.SQLiteInMemoryMode.SeviceTests.IssueServiceTests.Integration
{
	...
	{		
		...
		public async Task GetIssueProductSummaryById_GetData_ReturnDTO()
		{
			...
			Assert.Equal(400, result.Result	.Items.FirstOrDefault(x => x.ProductId == 11).Quantity);
			Assert.Equal(150, result.Result.Items.FirstOrDefault(x => x.ProductId == 10).Quantity);
		}
		[Fact]
		public async Task LoadingIssueAsync_ProperData_ReturnList()
		{
			...
			var palletQ1000 = result.Result.Pallets.First(p => p.PalletId == "Q1000");
			...
			// --- Paleta Q2000 ---
			var palletQ2000 = result.Result.Pallets.First(p => p.PalletId == "Q2000");
